...
const RatingForClass = () => {
  ...
  const sortedUsers = [...dataTable].sort((a, b) => b.bonusPoints - a.bonusPoints);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.blockRating}>
        ...
        <button className={styles.buttonRatting} onClick={() => navigate('/rating/classes')}>Вернуться к выбору класса</button>
      </div>
      <div className={styles.tableContainer}>
        {dataTable.length > 0 ? (
          <>
            <TableForClasses users={getCurrentUsers()} currentPage={currentPage} />
            <Pagination
